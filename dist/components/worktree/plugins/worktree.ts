...
import type { OpencodeClient } from "./nttcoder-primitives/types"
...
import { getProjectId } from "./nttcoder-primitives/get-project-id"
import {
...
async function loadWorktreeConfig(directory: string, log: Logger): Promise<WorktreeConfig> {
	...
	try {
		...
		if (!(await file.exists())) {
			...
			const defaultConfig = `{
  "$schema": "https://registry.nttcoder.dev/schemas/worktree.json",

  // Worktree plugin configuration
  // Documentation: https://github.com/nttcoderkenny/ocx
